...
def draw_tl_center(image_in, center, state):
    ...
    #print(center)
    x,y = center
    cv2.circle(img_copy, (x, y), 2, (0,0,0), 5)

    # Write text
    font                   = cv2.FONT_HERSHEY_SIMPLEX
    bottomLeftCornerOfText = (x-110,y-20)
    fontScale              = 0.4
    fontColor              = (0,0,0)
    lineType               = 1
    text = '({0}, {1})'.format(center,state)
...
def mark_traffic_signs(image_in, signs_dict):
    ...
    for sign in signs_dict:
        coord = signs_dict[sign]
        img_copy = draw_tl_center(img_copy,coord,sign)
    return img_copy
