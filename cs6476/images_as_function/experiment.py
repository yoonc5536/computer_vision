...
def main():
    img3_filename = "southafricaflagface.png"
    img3 = cv2.imread(img3_filename)
    img3_green = extract_green(img3)
    img3_blue = extract_blue(img3)
    img3_red = extract_red(img3)
    cv2.imwrite('output/sf_green.png', img3_green)
    cv2.imwrite('output/sf_blue.png', img3_blue)
    cv2.imwrite('output/sf_red.png', img3_red)
    gray_image = cv2.cvtColor(img3, cv2.COLOR_BGR2GRAY)
    cv2.imwrite('output/sf_grey.png', gray_image)
    ...
    # Note to TA: it says at least 100, shouldn't it include 100?
    ...
    #cv2.imwrite('output/ps1-4-b-1.png', normalized_img.astype(dtype=np.uint8))
